...
> [!info] 설계안 · 2026-09-09
> 대한민국 자료 37건을 기반으로 호주·미국·중국·유럽까지 확장한다. **G1 국가별 메뉴·국제 Metadata·문서 및 원문 검색 필터를 v0.6에서 구현했다.** 해외 문서 수집과 정기 실행은 후속 단계다. 아래 설명은 AI가 공식 자료원을 조사해 작성한 설계이며 사람 검수는 대기 상태다.
...
G1 구현 현황: 국가·관할·시장·원문 언어·문서 종류·원어/한국어 제목·식별번호·연도판·버전을 v1.1 표준으로 추가했다. 현재 적용 규칙은 [Frontmatter 표준](https://github.com/Uptec-khj/gridex-power-policy-kb/blob/main/docs/frontmatter-standard.md)에 기록한다. 아래 표 중 법적 효력·시행 상태·번역 검수·이용조건 상세 필드는 후속 과제로 남아 있다.
...
v1.1에서는 국내 계획 계열·차수 검증을 유지하면서 외국 연도판 계획과 계열 없는 규정·가이드를 지원한다. 기술 문서 검색은 `document_type`으로 구분한다. 기존 37건의 문서 ID·본문·원본 해시·공개 주소·관계 링크 보존을 확인했다. 원어 제목은 기존 편집 제목을 공식 제목으로 추정하지 않고 국내 미대조 항목은 null로 이관했다.
...
| 단계 | 개발 결과 | 완료 기준 |
| --- | --- | --- |
| G0 · 범위·자료원 설계 · 완료 | 이 로드맵과 비활성 후보 목록 | 네 지역 자료원·메뉴·수집 흐름·남은 검증 명시 |
| G1 · 국가 탐색·표준 · 완료 | 국가/지역 허브, 관할·언어·문서 종류 Metadata, 공통 필터 | 한국 37건의 주소·해시·검색 유지, 빈 지역 표시, 잘못된 관할/계열 조합 차단 |
...
바로 다음 개발 작업은 **G2: 호주 6건의 공식 원문·이용조건 확인과 수집 파일럿**이다. 시행 상태·번역·공개 권리의 실제 사례를 확인하고 상세 표준을 보강한 뒤 수집·검색·검수의 전체 경로를 검증한다. 국내 원문 복원과 [[review-queue|사람 검수 대기 작업]]도 별도 완료 조건으로 계속 유지한다.
